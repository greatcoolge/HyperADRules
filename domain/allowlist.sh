...
log_file="script.log"  # 添加日志文件

# 下载并合并allowlist文件
echo "Downloading allowlist URLs..." | tee -a "$log_file"
> "$allowlist_tmp"  # 清空文件
...
# 清理空行和空格
echo "Cleaning up domain list..." | tee -a "$log_file"
sed -i '/^$/d' domain.txt
sed -i 's/[[:space:]]//g' domain.txt

echo "Cleaning up invalid rules list..." | tee -a "$log_file"
sed -i '/^$/d' invalid_rules.txt
sed -i 's/[[:space:]]//g' invalid_rules.txt
sort -u invalid_rules.txt -o invalid_rules.txt

# 添加 adblock 前缀
echo "Adding @@||^ prefix to domain list..." | tee -a "$log_file"
while read -r domain; do
    echo "@@||$domain^"
done < domain.txt > adblocker_with_prefix.txt

# 完成提示
echo "Pure domain list generated in 'domain.txt'." | tee -a "$log_file"
echo "Invalid rules saved in 'invalid_rules.txt'." | tee -a "$log_file"
echo "Adblocker list with @@||^ prefix saved in 'adblocker_with_prefix.txt'." | tee -a "$log_file"
